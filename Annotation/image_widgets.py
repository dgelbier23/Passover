...
class ImageImport(ctk.CTkFrame):
    def __init__(self,parent, import_func):
        ...
        self.title_label.grid(row=0, column=0, columnspan=9, pady=10)  # Span across all columns, add some padding
    ...
    def configureGrid(self):
        # Configure grid to have 9 columns with even weight
        for i in range(9):
            self.columnconfigure(i, weight=1)

    def createImageList(self):
        # populate list for image values
        for infile in glob.glob("./01_base_images/*.jpeg"):
    ...
    def createButtons(self):
        ...
        col_indices = {BUTTON_BASE: [0, 1, 2], BUTTON_EDIT: [3, 4, 5], BUTTON_COMPLETE: [6, 7, 8]}

        for ima in self.button_images:
            # Determine the column and row for the current button
            col = col_indices[ima['status']][col_counters[ima['status']] % 3]
            row = 1 + (col_counters[ima['status']] // 3) * 2
...
class CloseOutput(ctk.CTkButton):
    def __init__(self, parent,close_func, path = None):
        super().__init__(
            master = parent,
            command = close_func,
            text = 'x',
